    ...
    >
      ...
      <div className="flex items-center space-x-1 min-w-0">
        ...
        <span className="text-xs text-gray-600 font-medium truncate">
          {task.estimatedTime ? formatEstimatedTime(task.estimatedTime) : 'Keine Planzeit'}
        </span>
      </div>
...
export function TaskBoard() {
  ...
  // ✨ CRITICAL: Precise sensors to prevent springing
  const mouseSensor = useSensor(MouseSensor, {
    activationConstraint: {
      distance: 0, // Start immediately on mouse down
    },
  });
  
  const touchSensor = useSensor(TouchSensor, {
    activationConstraint: {
      delay: 0, // Start immediately on touch
      tolerance: 0,
    },
  });
  ...
  const [originalExpandedProjects, setOriginalExpandedProjects] = useState<Set<string> | null>(null);
  ...
  const getProjectTasks = useMemo(() => {
    ...
    const projectTasks = projectColumns.map(project => {
      ...
      if (tagFilters.length > 0) {
        tasks = tasks.filter(task => 
          tagFilters.some(tagFilter => task.tags.includes(tagFilter))
        );
      }
    ...
    return projectTasks;
  }, [state.tasks, state.columns, priorityFilter, tagFilters]);
  ...
  // Using default collision detection for precise mouse positioning
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (task) {
      ...
      // Check if this is a task from a date column (not a sidebar task)
      const isSidebarTask = event.active.data.current?.type === 'sidebar-task';
      const isDateColumnTask = !isSidebarTask && task.columnId?.startsWith('date-');
      
      // If dragging from date column, collapse all projects to highlight drop zones
      if (isDateColumnTask) {
        console.log('TaskBoard: Collapsing projects for date column task drag to show clear drop zones');
        setOriginalExpandedProjects(new Set(expandedProjects));
        // Collapse all projects so only headers are visible as drop targets
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (newOverId !== overId) {
      console.log('TaskBoard: Drag over changed:', { 
        ...
        isTopZone: newOverId?.endsWith('-top'),
        isColumn: newOverId ? state.columns.some(col => col.id === newOverId) : false,
        isTask: newOverId ? state.tasks.some(task => task.id === newOverId) : false,
        availableColumnIds: state.columns.map(col => col.id).slice(0, 10) // Show first 10 to avoid spam
      });
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Restore original expanded projects state if it was saved during drag start
    if (originalExpandedProjects !== null) {
      console.log('TaskBoard: Restoring original expanded projects state');
      setExpandedProjects(originalExpandedProjects);
      setOriginalExpandedProjects(null);
    }
  };
  ...
  const handleDragCancel = () => {
    ...
    // Restore original expanded projects state if it was saved during drag start
    if (originalExpandedProjects !== null) {
      console.log('TaskBoard: Restoring original expanded projects state after cancel');
      setExpandedProjects(originalExpandedProjects);
      setOriginalExpandedProjects(null);
    }
  };
          ...
          <div 
            className={`absolute top-0 left-0 bottom-0 w-80 z-20 flex flex-col overflow-hidden min-w-0 ${
              isMinimalDesign
                ...
                : 'bg-[#23262A] border-r border-gray-800'
            }`}
            style={{
              ...(isMinimalDesign 
                  ? { 
          ...
          >
            ...
            <div 
              className={`flex flex-col justify-center px-4 min-w-0 ${
                isMinimalDesign
                  ...
                  : 'border-b border-gray-800 bg-[#23262A]'
              }`}
              style={{ 
                ...(isMinimalDesign 
                    ? { 
            ...
            >
              <h1 className={`text-lg font-semibold flex items-center space-x-2 min-w-0 ${
                ...
                <span className="truncate">{i18n.language === 'en' ? 'Planner' : 'Planer'}</span>
              </h1>
            </div>
              ...
              <div>
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-lg transition-all duration-300 ${
                    isMinimalDesign 
                      ? (showFilters || priorityFilter !== 'all' || tagFilters.length > 0
                  ...
                  <div className="p-4 rounded-lg bg-black/20 backdrop-blur-sm border border-gray-600/30 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-5 gap-1">
                        ...
                        {[
                          { key: 'high', label: 'H', color: '#ef4444', title: 'Hohe Priorität' },
                          { key: 'medium', label: 'M', color: '#f59e0b', title: 'Mittlere Priorität' },
                          { key: 'low', label: 'L', color: '#10b981', title: 'Niedrige Priorität' },
                          { key: 'none', label: '–', color: '#9ca3af', title: 'Keine Priorität' }
      ...
      : (i18n.language === 'en' ? 'No tasks' : 'Keine Aufgaben');
          ...
          >
            ...
            >
              {activeTask ? (
                (() => {
                  const isSidebarTask = state.activeTimer?.taskId === activeTask.id || 
                    getProjectTasks.some(group => group.tasks.some(t => t.id === activeTask.id));
                  ...
                  return (
                    ...
                    >
                      {isSidebarTask ? (
                          <div className="bg-white/95 backdrop-blur-sm rounded-lg border border-gray-200/50">
                          <SidebarTaskItem task={activeTask} formatEstimatedTime={formatEstimatedTime} />
                        </div>
                      ) : (
                        <TaskCard task={activeTask} isInDragOverlay />
                      )}
